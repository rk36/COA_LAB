...
void fillreadyqueue(queue<process>& waiting_queue,queue < process >& ready_queue,vector< vector<int> >& vec, vector<int>& process_pos);
void running(queue<process>& waiting_queue,queue < process >& ready_queue,vector< vector<int> >& vec, vector<int>& process_pos);

int global_time;
bool run;


void printqueue(queue <process> p)
{
...
vector<int> process_pos(num_of_process+1);
...
cout<<"\n average turnaround time "<<sum/(vec.size())<<endl;
...
void fillreadyqueue(queue<process>& waiting_queue,queue < process >& ready_queue,vector< vector<int> >& vec, vector<int>& process_pos)
{
    ...
    for(int i=0;i<vec.size();i++)
     {
         if(process_pos[i] == 0 && vec[i].size()>1 && vec[i][0] == global_time)
         {
             p.time = min(time_quantum,vec[i][1]);
             vec[i][1]  = vec[i][1] > time_quantum?vec[i][1] - time_quantum:0; 
             p.id = i;
             ready_queue.push(p);
    ...
    while(!waiting_queue.empty())// && temp.time == global_time)
    {
      ...
      p.time = min(time_quantum,vec[temp.id][process_pos[temp.id]]);  // insert in ready queue if the process is of current time stamp;
      vec[temp.id][process_pos[temp.id]] = vec[temp.id][process_pos[temp.id]] > time_quantum ? vec[temp.id][process_pos[temp.id]] - time_quantum:0;
      p.id = temp.id;  
      ready_queue.push(p);
...
void running(queue<process>& waiting_queue, queue< process>& ready_queue,vector< vector<int> >& vec, vector<int>& process_pos)
{
    ...
    special_process.id = vec.size()+1;
    special_process.time = 1;
    ...
    if(running_time == 0 || running_time == -1)     //&& ready_queue.empty())
    {
       ...
       if(vec[p.id][process_pos[p.id]] == 0 && process_pos[p.id]+1 < vec[p.id].size() && running_time != -1)
       {
       ...
       else if(vec[p.id][process_pos[p.id]] != 0 && running_time != -1)
       {
           temp_process = p;
           temp_process.time = min(time_quantum,vec[p.id][process_pos[p.id]]);
           vec[p.id][process_pos[p.id]] = vec[p.id][process_pos[p.id]] > time_quantum ?vec[p.id][process_pos[p.id]] - time_quantum:0; 
           ready_queue.push(temp_process); 
           cout<<global_time<<"::"<<"process-> "<<p.id<<" run_2_ready "<<endl;
           entry_to_ready[p.id] = global_time;
       }
       else if(vec[p.id][process_pos[p.id]] == 0 && running_time != -1)
       {
     ...
     ready_queue.pop();
     running_time = min(time_quantum,p.time);
     running_time--;
